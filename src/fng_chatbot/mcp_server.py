...
class FngMcpService:
    ...
    def send_telegram_report(
        ...
        *,
        ...
        """Send an exact reviewed preview only when delivery is explicitly enabled."""

        if not self._allow_telegram_send:
            raise PermissionError(
                "Telegram sending is disabled until an authenticated deployment enables it"
            )
        if confirm_send is not True:
            raise PermissionError("confirm_send=true is required for Telegram delivery")
        ...
        result = self._telegram_service.send_preview(
            preview,
...
def create_mcp_server(service: FngMcpService) -> FastMCP:
    ...
    server = FastMCP(
        ...
        instructions=(
            "Inspect the seven-indicator report and data quality before previewing. "
            "Call send_telegram_report only after explicit user confirmation."
        ),
        json_response=True,
    )
    ...
    def send_telegram_report(
        ...
        """Send an exact reviewed preview after explicit confirmation and enablement."""
...
def create_default_service() -> FngMcpService:
    ...
    return FngMcpService(
        ...
        allow_telegram_send=_env_flag("MCP_ALLOW_TELEGRAM_SEND"),
    )
