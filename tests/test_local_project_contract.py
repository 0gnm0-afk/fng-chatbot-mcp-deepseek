...
def test_codex_and_claude_start_the_same_stdio_module() -> None:
    ...
    assert codex["tools"]["send_telegram_report"]["approval_mode"] == "auto"
    assert set(codex["env_vars"]) == EXPECTED_ENVIRONMENT_VARIABLES
    assert claude["env"] == {
        "NVIDIA_API_KEY": "${NVIDIA_API_KEY:-}",
        "TELEGRAM_BOT_TOKEN": "${TELEGRAM_BOT_TOKEN:-}",
        "TELEGRAM_CHAT_ID": "${TELEGRAM_CHAT_ID:-}",
        "MCP_ALLOW_TELEGRAM_SEND": "${MCP_ALLOW_TELEGRAM_SEND:-true}",
...
def test_codex_and_claude_skills_share_one_contract() -> None:
    ...
    assert "send the generated Telegram report by default" in codex_skill
    assert "No separate user approval turn is required" in codex_skill
    assert "preview-only" in codex_skill
