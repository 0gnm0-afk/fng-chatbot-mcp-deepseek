...
def test_send_is_blocked_when_server_lock_is_disabled() -> None:
    sender = FakeSender()
    service, snapshots = build_service(sender=sender)
...
def test_send_builds_and_delivers_current_preview_without_confirmation() -> None:
    sender = FakeSender()
    markets = FakeMarketOverviewProvider()
    interpretations = FakeInterpretationProvider()
    service, snapshots = build_service(
        sender=sender,
        market_overview_provider=markets,
        interpretation_provider=interpretations,
        allow_send=True,
    )
    ...
    telegram = result["telegram"]
    assert telegram["sent"] is True
    assert telegram["message_id"] == 789
    assert telegram["idempotency_key"] == "report:2026-07-20"
    assert telegram["preview_hash"] == hash_telegram_preview(telegram["text"])
    assert sender.calls[0]["text"] == telegram["text"]
    assert snapshots.calls == [True]
    assert markets.calls == 1
    assert result["interpretation"]["status"] == "applied"


def test_telegram_send_flag_defaults_on_but_explicit_false_still_locks(monkeypatch) -> None:
    monkeypatch.delenv("MCP_ALLOW_TELEGRAM_SEND", raising=False)
    assert _env_flag("MCP_ALLOW_TELEGRAM_SEND", default=True) is True

    monkeypatch.setenv("MCP_ALLOW_TELEGRAM_SEND", "false")
    assert _env_flag("MCP_ALLOW_TELEGRAM_SEND", default=True) is False
...
async def test_mcp_client_lists_three_tools_and_calls_structured_report() -> None:
    ...
    send_properties = by_name["send_telegram_report"].inputSchema["properties"]
    assert set(send_properties) == {"idempotency_key", "force_refresh"}
