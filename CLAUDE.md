# fng-chatbot — Claude Code 진입점

작업 전 `README.md`와 `설계.md`를 읽고 변경 범위와 검증 명령을 확인한다. 작업 후 관련 테스트와 Ruff 검사를 실행하고 결과를 기록한다.
...
- `.claude/skills/fear-greed-report/SKILL.md` — Claude Code 분석 절차
- `.mcp.json` — 프로젝트 stdio MCP 연결
...
- Telegram은 보고서 실행 시 기본 전송하며, `MCP_ALLOW_TELEGRAM_SEND=false`를 명시하면 잠근다. 전송 원문은 내부 생성 미리보기와 SHA-256이 일치해야 한다.
...
Skill을 수정하면 두 Skill 본문이 같은지와 실제 MCP 도구 이름이 일치하는지 확인한다.
